...
@implementation EDNDataReaderState
...
@interface EDNStreamReaderState ()

-(void)checkStreamAndBufferStatus;

@end

@implementation EDNStreamReaderState
    ...
    if (_characterBufferLength == 1) _currentCharacter = (unichar)_characterBuffer[0];
        ...
        for (int i = 1; i < _characterBufferLength; i++) {
            if (!(_characterBuffer[i] & 0x80)) {
                self.error = EDNErrorMessage(EDNErrorInvalidData, @"Invalid UTF8 encountered in stream.");
                return;
            }
            _currentCharacter |= ((unichar)(_characterBuffer[i] & 0x3F)) << (6 * (_characterBufferLength - i));
        }
    } else {
        self.error = EDNErrorMessage(EDNErrorInvalidData, @"Unable to parse Unicode points beyond U+FFFF at this time.");
    }
}
